...
<div id="solidres">
    <div class="row-fluid">
		...
		<div id="sr_panel_right" class="sr_list_view span10">
			<form action="<?php echo JRoute::_('index.php?option=com_solidres&view=taxes'); ?>" method="post" name="adminForm" id="adminForm">
				<table class="table table-striped">
					<thead>
						<tr>
							...
							<th class="title">
								<?php echo JHtml::_('grid.sort',  'SR_HEADING_RATE', 'r.rate', $listDirn, $listOrder); ?>
							</th>
							<th>
					...
					<?php foreach ($this->items as $i => $item) :
						...
						$canCreate	= $user->authorise('core.create',		'com_solidres.tax.'.$item->id);
						$canEdit	= $user->authorise('core.edit',			'com_solidres.tax.'.$item->id);
						$canCheckin	= $user->authorise('core.manage',		'com_checkin') || $item->checked_out==$user->get('id') || $item->checked_out==0;
						$canChange	= $user->authorise('core.edit.state',	'com_solidres.tax.'.$item->id);
						?>
						<tr class="row<?php echo $i % 2; ?>">
							...
							<td>
								<?php if ($canCreate || $canEdit) : ?>
									<a href="<?php echo JRoute::_('index.php?option=com_solidres&task=tax.edit&id='.(int) $item->id); ?>">
							...
							<td class="center">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'taxes.', $canChange);?>
							</td>
						</tr>
						<?php endforeach; ?>
					</tbody>
