...
class profileModelprofile extends JModel
{
	...
	function checkout($uid = null)
	{
		if ($this->_id)
		{
			...
			$posts_detail = $this->getTable('redsocialstream_profile');
	...
	function _loadData()
	{
		...
		if (empty($this->_data))
		{
			$query = '
				...
				LEFT JOIN  #__redsocialstream_profiletype AS p on h.profiletypeid = p.id
				LEFT JOIN  #__redsocialstream_group AS g on h.groupid = g.id
				WHERE h.id = ' . $this->_id;
	...
	function delete($cid = array())
	{
		if (count($cid))
		{
			...
			$query = 'DELETE FROM #__redsocialstream_profilereference WHERE id IN ( ' . $cids . ' )';
			$this->_db->setQuery($query);
			if (!$this->_db->query())
			{
				$this->setError($this->_db->getErrorMsg());
	...
	function publish($cid = array(), $publish = 1)
	{
		...
		if (count($cid))
		{
			...
			$query = 'UPDATE #__redsocialstream_profilereference'
